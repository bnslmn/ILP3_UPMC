...
#include <string.h>
#include <setjmp.h>
// ajout de la bibliothèque standard POSIX
#include <ucontext.h>
...
typedef struct ILP_Object {
     ...
     union {
          ...
          struct asCoroutine {                           // ******** TME 7 BY AMINE BENSLIMANE *********
...
#define ILP_CheckIfCoroutine(o) \
  if ( ! ILP_isCoroutine(o) ) { \
       ILP_domain_error("pas une coroutine !", o); \
  };
// *****************************************************
...
// Ajout de ILP_object_Coroutine_class
